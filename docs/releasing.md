...
Current capture uses a real OpenGL window and is intentionally not claimed by generic hosted CI. The manually dispatched `Minecraft integration` workflow is pinned to a protected self-hosted runner carrying the `macOS` and `packwright-client-capture` labels. It fails closed unless the runner process belongs to the active graphical console user and has an Aqua bootstrap session. On that trusted machine, use an isolated absolute workspace/cache and Java 25, explicitly accept the EULA, and run `setup-version 26.2 --client-capture`. Capture known item, native-block, block-display, equippable-head fallback/camera-overlay plus an exact chest-armor compatibility stack, equipment-model head layer, a zombie-backed idle/walk/attack core, exact allow-listed native-entity variant/component/equipment entries, a mixed block-display/item-display rig, floor-only native placeables, and simulated placeables from exact proposals/representations. Exercise all three transparent-block backgrounds/copy layouts and exact block-light-source placement. Confirm required scenes contain no injected/debug aid; supplemental scale/hitbox/matched empty-subject controls appear only in the QA sheet; resource/model readiness and bounded settling are recorded; and actual-client metrics bind source PNG hashes. Run `visual_validate` with its default capture-required policy, and prove `visual_commit` accepts only each exact `expectedClientCaptureReportSha256`.

The repository acceptance harness exercises the held-item vertical slice plus the implemented block, headwear, entity, and placeable representation strategies with default and conditional scenes:
...
Add `PACKWRIGHT_KEEP_INTEGRATION_WORKSPACE=true` only when a local reviewer needs the authoritative PNG/report files. The harness logs the retained randomized absolute path; delete it after review, and never retain or upload it from CI.

Review that each result binds the expected client/mod/pack/representation/studio hashes, includes the complete required/supplemental partition, records the OS/Java/GPU/driver/OpenGL/settings environment, and has a clean resource-load log. Every artifact must include the exact live-client `observedFixture` and its independently verified canonical hash. Required stock equipped-subject views and ordinary scale context must report `authoritative_environment_capture`; bare-head/injected scale/comparison, debug-hitbox, matched empty-subject controls, and non-ordinary scale aids must report `augmented_qa_reference` and remain supplemental. Tamper representation/proposal/settings/report/observed-fixture hashes, stale proposal bindings, and required/supplemental partitions and confirm rejection. Confirm supplemental controls cannot satisfy authority or block acceptance; native placeable wall/ceiling declarations and mismatched `facing` properties fail; unsupported entity types/states and command/function/save/path/mod/credential fields fail before launch; timeout/cancellation cleanup runs; and no user save or unrelated mod is touched. Authoritative pixels are release evidence for that recorded environment, not a cross-GPU golden image.

Destroy the isolated game/cache state when review is complete. Do not upload Mojang artifacts, full logs, or framebuffer evidence to GitHub Actions. The protected self-hosted workflow records only pass/fail status; it must never add artifact-upload steps for the cache, full logs, or framebuffers.
