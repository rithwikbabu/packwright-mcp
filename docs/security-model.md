...
`visual_capture` requires `confirm: true`, exact immutable run/proposal identities, prepared local artifacts, Java 25, and a graphical-session preflight. The launcher uses a fixed Fabric `KnotClient` entrypoint and fixed offline-developer, multiplayer-disabled, and chat-disabled arguments. It stages only the exact proposal plus the bundled Packwright capture mod in a new disposable game directory. The protocol accepts bounded scene fields and the exact generated item stack—not credentials, arbitrary commands, a user save, an external mod, JVM flags, or shell input.

The capture mod writes fixed-name PNGs, a canonical report, bounded full log, and final completion sentinel. Packwright follows no output symlinks and verifies every report/scene/input/hash relationship before indexing evidence. Cancellation, timeout, failure, and success all trigger cleanup of the disposable game directory. Captures use the actual OpenGL framebuffer and therefore reveal the recorded GPU/driver/environment; they are authoritative for that environment, not a cross-hardware reproducibility promise.
