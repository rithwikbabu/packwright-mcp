...
- A generic scene-review profile layer over the deterministic CPU renderer. The first profile, `held_item@1`, renders held-item scenes with reference arms/bodies and produces an immutable advisory measurement report.
- Immutable repair revisions, including constrained held-item metadata repairs, guarded multi-file commit, layered validation, and deterministic paired builds.
...
`reviewProfile` selects how the compiled model is staged for review; it never changes the Minecraft model JSON. The only implemented profile is `held_item`. Its optional `heldItem` metadata describes a model-space primary grip, optional secondary grip and muzzle, optional forward axis, handedness, item kind, two-handed intent, and use pose. A two-handed item must declare `secondaryGrip`; `aim` must declare a muzzle or forward axis. Omitting `heldItem` keeps older specifications readable and reports the unavailable semantic measurements as `skipped` rather than inventing a pass. Other configured profile checks still run, and a failed check blocks readiness.

## Review profiles

A review profile owns its required and conditional scenes, reference geometry, cameras, reusable measurement rules, and warning/failure thresholds. Minecraft display transforms remain the compiled output. The profile applies those exact transforms to the item while independently posing Packwright-authored reference arms and bodies; reference geometry is never written to the resource pack.
...
8. If review finds a defect, call `visual_revision_create` with the parent revision ID, its exact `expectedSpecSha256`, a human-readable finding, and only the named part, material, display-transform, or held-item metadata repairs. Compile and render the child revision, then compare the same profile scenes.
...
10. After explicit acceptance, call `visual_commit` with `confirm: true` and the exact current `proposalSha256`. Commit also requires a current `held_item@1` report with no failed measurements and complete semantic metadata.
...
Runs live under `<cache>/visual-runs/<run-id>/`. The run ID is derived from canonical request, specification, and provenance hashes. Each repair creates a content-addressed child under `revisions/<revision-id>/`; existing run and revision directories are never edited in place. Textures, compiled proposals, renders, profile reports, and targeted repair records are stored by content hash, while `<cache>/visual-project-state/` indexes the one active head for each project. A profile report binds its spec hash, compiled-artifact ID, renderer/profile versions, scene-plan hash, required view IDs, individual image hashes, measurements, and readiness result. Changing the spec, compiled artifact, profile plan, or renderer makes older evidence stale instead of silently reusing it.
...
The selected profile determines the scene set. `held_item@1` uses the 12 required and up to four conditional scenes listed above instead of a universal turntable. Individual views are exposed as PNG resources, and the immutable JSON report is exposed separately. The contact sheet is at most 720 KiB and is returned as MCP image content; render size is 32–256 pixels, profiles are capped at 16 scenes, model scenes are capped at 512 authored parts plus bounded reference geometry, and raster work is bounded.

The renderer is a fast deterministic review aid, not proof of exact client rendering. Arm/torso intersection is measured with transformed axis-aligned bounding boxes rather than exact mesh collision, cameras use representative FOVs and poses, and conditional scenes pose the base compiled geometry rather than resolving every active item-state model. Special models marked unsupported by the profile require later real-client capture. The current release has no actual-client screenshot harness and no block, placeable, armor, head-wearable, projectile, GUI-item, or entity-model review profile.

## Transactional commit and build

`visual_connect` records the SHA-256 or expected absence of every destination. Before changing a pack, `visual_commit` also verifies that the same compiled proposal has a current, complete `held_item@1` report with no failed advisory measurement. It then:
...
`visual_validate` reports these layers independently: paired metadata, schema, texture, asset graph, geometry/UV, profile render readiness, binding, vanilla commands, and GameTests. It verifies the immutable render report against the current spec, compiled artifact, profile plan, required view hashes, and renderer/profile versions. It applies the exact uncommitted proposal to stable full-pack snapshots before resource-pack, command, and GameTest checks, so validation evidence covers the bytes proposed for commit rather than only the already committed packs. Semantic diagnostics name an asset and, where possible, its exact part, material, display context, review profile, scene, or measurement.

The current automated visual checks cover strict schema/canonical compilation, required textures, zero-area UV assignments, material/tint conflicts, part relationships, approximate display clipping, graph endpoints/cycles/orphans, the `minecraft:item_model` chain, and the `held_item@1` advisory measurements. Agent visual review still supplies aesthetic judgment; neither its judgment nor the software-rendered measurements are authoritative Minecraft results. Vanilla-backed command validation remains authoritative for datapack command syntax, while an optional GameTest supplies behavior evidence.
