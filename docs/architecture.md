...
The client-capture profile pins protocol version 1, Java 25, the OpenGL backend, Fabric Loader `0.19.3`, exact Fabric library hashes, capture-mod `0.4.0`, and a per-review-profile support map. Client setup compiles Mojang's version metadata and asset index into a platform-specific artifact manifest without changing public visual tool contracts.
...
`visualCapabilities` describes Minecraft's representation boundary separately from implementation readiness. `native`, `simulated`, `replacement`, and `requires_mod` are semantic claims, not marketing labels; `support` describes that vanilla boundary, while `compilerSupport` describes the current Packwright implementation. In v0.4, `compilerSupport` is `full` for `custom_item`, `limited` for `conditional_item_state`, and `unsupported` for every other target. `clientCaptureSupport` is a third boundary: it is `limited` for `held_item`, `full` for `gui_item`, and `unsupported` for the other profiles. The held-item limit rejects `twoHanded: true` until the adapter can pose and verify a secondary Minecraft-rendered arm at `secondaryGrip`. In particular, a carrier or display rig never becomes a native new block/entity identity merely because it can be previewed.
...
For supported one-handed `held_item` proposals and `gui_item`, the separate capture adapter lowers the resolved profile into a strict capture plan, snapshots both packs, applies the exact proposal, and launches the official Minecraft 26.2 client through Fabric Loader in a disposable game directory. It rejects `twoHanded: true` before launch because v0.4 cannot yet pose and verify a secondary Minecraft-rendered arm at `secondaryGrip`. First-person plans sign the Minecraft-rendered arm augmentation with `referenceArmPurpose: "scale_only"`; it supplies scale/occlusion context and does not turn the frame into an authoritative palm-to-grip measurement. The client-only Packwright mod waits for actual resource reload/model baking, configures only allow-listed scene fields, and captures the game's framebuffer with Minecraft's screenshot API. It writes every output followed by a canonical report and atomic completion sentinel. The parent process verifies the full evidence chain before recording `authoritative_environment_capture` authority. That authority is scoped to the reported OS, GPU, driver, OpenGL backend, settings, client/mod hashes, and pack/item inputs; unlike the CPU images, the framebuffer is not promised to be pixel-identical on other hardware.
