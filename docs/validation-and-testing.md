...
`requireClientCapture` is tri-state. Omit it to require already captured evidence for profiles with `full` or `limited` client support and skip the layer for `unsupported`; pass `false` for an explicitly advisory/fast validation; pass `true` to require capture even when that means an unsupported profile fails. Verification covers the exact current spec, representation, compiled/proposal artifacts, project manifest, complete pack snapshots, client/mod identities, studio/settings/environment, scene plan, reload/model readiness, settling evidence, report, exact live-client `observedFixture` readbacks and their canonical hashes, client-pixel measurements, log, and source PNG hashes. Each measurement binds an explicit `requiredForReadiness` value. A failed or skipped critical measurement makes client-capture readiness false and blocks commit; critical warnings and noncritical skips remain visible, while supplemental-only failures remain non-authoritative QA. A primary-scene metric that consumes a supplemental matched empty-subject control is necessarily noncritical, so supplemental evidence cannot become an acceptance gate indirectly. Validation never launches the graphical client; call `visual_capture` or the CLI `capture` command first. Required equipped-subject stock views and ordinary in-world scale context have `authoritative_environment_capture` authority for their recorded environment. Bare-head/injected scale/comparison, debug-hitbox, matched empty-subject controls, and non-ordinary world-scale aids have `augmented_qa_reference`, never satisfy the required scene set, and live only in the supplemental sheet.
...
A passing structural or Spyglass result is not proof that Minecraft can parse every command. Vanilla-backed validation closes that gap for command syntax and codecs, but it still does not execute commands or prove runtime state. Use a vanilla test for release-critical predicates, scheduled behavior, world-state interactions, and GameTest resources. Conversely, a GameTest run only exercises selected behavior; retain structural validation, command validation, and code review. Likewise, a passing CPU render is not proof that the real client resolves every parent, special model, atlas, or built-in asset exactly as shown. For supported item, block, headwear, entity, and placeable representations, client capture adds actual-renderer evidence scoped to its reported environment; it does not validate runtime datapack behavior, turn a simulation into a native identity, or promise cross-hardware pixels.

## Continuous integration

Ordinary hosted CI never downloads Minecraft artifacts. It can exercise the semantic compiler, safe PNG decoder, deterministic renderer/contact-sheet pixel hashes, strict client-capture protocol, runtime-manifest selection, graph checks, transaction failure/rollback behavior, and deterministic resource-pack ZIPs entirely from original fixtures. The repository's separate `Minecraft integration` workflow is manual, uses the protected `minecraft-client-capture` environment, requires explicit EULA acceptance, and targets only a trusted self-hosted runner labeled `macOS` and `packwright-client-capture`. A fail-closed guard requires the runner process to belong to the active graphical console user and to have an Aqua bootstrap session. The workflow never uploads the server/client jars, generated vanilla cache, logs, or framebuffer evidence. Its server path proves intentionally invalid commands are rejected by the 26.2 dispatcher, repairs them, passes an explicit `minecraft:always_pass` GameTest, builds a deterministic ZIP, and loads the extracted artifact through vanilla again. Its paired path sets `PACKWRIGHT_RUN_CLIENT_CAPTURE=true`, exercises the complete official-client representation matrix, validates the exact reports, commits accepted proposals, builds both packs deterministically, and performs the final vanilla load.

The official-client release gate remains separate from hosted CI because capture requires a real interactive macOS OpenGL session. The protected self-hosted workflow supplies that gate and refuses a non-console or non-Aqua runner. Maintainers should explicitly accept the EULA, select an isolated cache, and capture known item, native-block, block-display, all three transparent-block layouts, equippable-head fallback/camera-overlay with an exact chest-armor stack, equipment-model head layer, a zombie-backed idle/walk/attack core, exact allow-listed native-entity variants/components/equipment, a mixed display rig, floor-only native placeables, and simulated placeables with default and declared conditional profiles. Confirm required frames contain no injected/debug aid, supplemental hitbox/scale/matched-control frames cannot replace them, model-bake readiness and exact light-source/subject-light state are recorded, display fixtures settle for at least two rendered client ticks, every artifact contains a verified live-client observed-fixture hash, every non-skipped client metric binds source PNGs, and uncalibrated geometry metrics are explicitly skipped. Tamper each representation/proposal/settings/report/observed-fixture/hash binding and the required/supplemental partition in negative tests; reject native placeable wall/ceiling bindings, facing mismatches, unsupported entity types/states, and every command/function/save/path/mod/credential field before launch. Run `visual_validate` with its default capture-required policy and commit with each exact report hash. Never upload Minecraft artifacts, full logs, or framebuffers as workflow artifacts.
...
For local evidence inspection only, add `PACKWRIGHT_KEEP_INTEGRATION_WORKSPACE=true`. The harness prints the absolute randomized temporary workspace and leaves it in place instead of deleting it. Review the framebuffer/report files there, then delete that disposable workspace yourself. Never enable retention in CI or publish its Minecraft-derived evidence.
